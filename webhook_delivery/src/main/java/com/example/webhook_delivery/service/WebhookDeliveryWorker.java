...
public class WebhookDeliveryWorker {
    ...
    public void handleDelivery(WebhookMessage message) {
        ...
        if (message.getEventType() == null) {
            logger.warn("⚠️ Missing eventType for task {} — skipping.", message.getDeliveryTaskId());
            return;
        }

        List<Subscription> subscriptions = subscriptionService.findByEventType(message.getEventType());
